...
class FieldOdometry:
    ...
    # def weighted_pose_average(self) -> Pose2d:
    #     """
    ...
    def update(self):
        ...
        self.visionPose = self.drivetrain.estimator.getEstimatedPosition()

        weightedPose = Pose2d(
            self.visionPose.X() * config.kRobotVisionPoseWeight
            + self.robot_pose.X() * (1 - config.kRobotVisionPoseWeight),
            self.visionPose.Y() * config.kRobotVisionPoseWeight
            + self.robot_pose.Y() * (1 - config.kRobotVisionPoseWeight),
            self.robot_pose.rotation(),
        )

        self.drivetrain.odometry.resetPosition(weightedPose, self.robot_pose.rotation())
