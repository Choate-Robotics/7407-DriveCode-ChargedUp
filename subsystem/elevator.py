...
class Elevator(Subsystem):  # elevator class
    ...
    pose: Pose3d
    #elevator_lengt                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               h: float # the length of the elevator in meters
    ...
    def is_at_position(self, position: Pose3d):
        ...
        if (self.get_pose() < Pose3d(position.X - constants.arm_pose_accuracy), position.Y - constants.arm_pose_accuracy, position.Rotation - constants.arm_pose_accuracy) and (self.get_pose() > Pose3d(position.X + constants.arm_pose_accuracy), position.Y + constants.arm_pose_accuracy, position.Rotation + constants.arm_pose_accuracy):
            return True
        else:
            return False

    def shoulder_angle_to_motor_rotations(self, radians):
        ...
        """
        return (radians * constants.elevator_rotation_gear_ratio) / (2 * math.pi)
    ...
    def init(self):  # initializing motors
        ...
        rev.SparkMaxLimitSwitch.Type.kNormallyOpen)
        self.abs_encoder = self.secondary_rotation_motor._motor.getAbsoluteEncoder(rev._rev.SparkMaxAbsoluteEncoder.Type.kDutyCycle)
        ...
        #     self.main_rotation_motor._motor.setSoftLimit(rev.CANSparkMax.SoftLimitDirection.kReverse, self.angle_to_shoulder_rotations(constants.shoulder_min_rotation) )
        self.main_rotation_motor._motor.setSoftLimit(rev.CANSparkMax.SoftLimitDirection.kForward, self.shoulder_angle_to_motor_rotations(constants.shoulder_max_rotation) )
        self.main_rotation_motor._motor.setSoftLimit(rev.CANSparkMax.SoftLimitDirection.kReverse, self.shoulder_angle_to_motor_rotations(constants.shoulder_min_rotation) )
    ...
    def boundary_box(self, angle: float) -> float: #TESTED
        ...
        if self.extension_overide:
            # if the extension overide is enabled, return 1, which will maximize the max elevator height
            return 1
        else:
            RNangle = angle
            angle = abs(angle)
            ...
            top_vertical_boundry_from_pivot = constants.vertical_boundary - constants.pivot_point_height - constants.top_boundry_buffer_gap #positive z
            bottom_vertical_boundry_from_pivot = constants.pivot_point_height - constants.bottom_boundry_buffer_gap #negative z
            horizontal_boundry_length_from_pivot = 0.5 * (constants.robot_length) + constants.horizontal_boundary - constants.side_boundry_buffer_gap #positive x
            
            # the max angle of the boundry dimsensions
            ...
            # if the angle is between the top boundry max angle and the bottom boundry max angle, return the length ratio of the angle and the horizontal boundry
                ...
                return get_length_ratio(90 - angle, horizontal_boundry_length_from_pivot)
            # if the angle is greater than the bottom boundry max angle, return the length ratio of the angle and the bottom boundry
                ...
                return get_length_ratio(180 - angle, bottom_vertical_boundry_from_pivot)
            else:
                return 1
    ...
    def set_length(self, meters):  # set arm extension 
        ...
        def set(length):
            start_length = (self.motor_extend.get_sensor_position() * constants.elevator_extend_gear_ratio) / constants.max_elevator_height
    ...
    def set_rotation(self, radians: float):  # set arm rotation
        ...
        def set(radians: float):
            rotations = (radians / (2 * math.pi)) * constants.shoulder_rotation_gear_ratio
    ...
    def get_rotation(self):  # returns arm rotation in radians
        ...
        return (self.main_rotation_motor.get_sensor_position() * constants.shoulder_rotation_gear_ratio) * (2 * math.pi)
    ...
    def zero_elevator_rotation(self):
        ...
        encoder_difference: float = abs_encoder_position - constants.elevator_initial_roatation
    ...
    def update_pose(self):
        ...
        self.pose = Pose3d(
            x, 0, z + constants.pivot_point_height, 0, 0, 0)
