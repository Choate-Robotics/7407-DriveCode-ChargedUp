...
class ZeroShoulder(SubsystemCommand[Arm]):
    ...
    def end(self, interrupted=False):
        if not interrupted:
            ...
            utils.logger.debug("SHOULDER", "Shoulder Zero Command Interrupted.")
...
class SetArmAuto(SubsystemCommand[Arm]):
    def __init__(
            self,
            subsystem: Arm,
            distance: meters,
    ...
    def end(self, interrupted: bool) -> None:
        if not interrupted:
            self.subsystem.enable_brake()
            commands2.CommandScheduler.getInstance().schedule(SetArm(self.subsystem, self.distance, self.real_desired))
